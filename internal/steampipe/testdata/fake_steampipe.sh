...
JSON
  ...
  *kubernetes_cluster_role_binding*)
    # Reproduces a real-world shape: a ClusterRoleBinding referencing a
    # ServiceAccount (here, the stock kube-system "bootstrap-signer")
    # that kubernetes_service_account never returns -- e.g. because the
    # ingest credential can list bindings cluster-wide but lacks `list`
    # on ServiceAccounts in kube-system. Exercises the auto-vivify path
    # in kubernetes_rbac.go rather than crashing AddEdge.
    cat <<'JSON'
{"columns":[{"name":"name","data_type":"text"},{"name":"role_name","data_type":"text"},{"name":"role_api_group","data_type":"text"},{"name":"role_kind","data_type":"text"},{"name":"subjects","data_type":"jsonb"}],
 "rows":[{"name":"system:controller:bootstrap-signer","role_name":"system:controller:bootstrap-signer","role_api_group":"rbac.authorization.k8s.io","role_kind":"ClusterRole","subjects":[{"kind":"ServiceAccount","name":"bootstrap-signer","namespace":"kube-system"}]}]}
JSON
    ;;
  *kubernetes_cluster_role*)
    cat <<'JSON'
{"columns":[{"name":"name","data_type":"text"},{"name":"rules","data_type":"jsonb"}],
 "rows":[{"name":"system:controller:bootstrap-signer","rules":[{"api_groups":[""],"resources":["configmaps"],"verbs":["get","update"]}]}]}
JSON
    ;;
  *)
    echo '{"columns":[],"rows":[]}'
